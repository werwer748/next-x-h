...
export default async function AfterLoginLayout({children, modal}: TProps) {
            ...
            <div className={style.leftSectionFixed}>
              <Link className={style.logo} href={session?.user ? "/home" : "/"}>
                <div className={style.logoPill}>
                  <Image src={ZLogo} alt={"z.com 로코"} width={40} height={40}/>
                </div>
              </Link>
              {session?.user &&
                  <>
        ...
        <div className={style.rightSectionWrapper}>
          <div className={style.rightSectionInner}>
            ...
            <section className={style.rightSection}>
              {/* 검색 폼이 클라이언트 컴포넌트로 사용되어야 하기 때문에 파일로 분리 */}
              <RightSearchZone/>
              <TrendSection/>
